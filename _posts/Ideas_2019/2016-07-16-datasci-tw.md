...
- Case 1: Wikipedia's Growth
  - 例如：怎麼量測維基百科的「整體性」的成長呢？將文章數量的成長繪圖出來，發現圖形是 exponential growth 的 pattern。能够以社會科學已知的 exponential growth 的機轉來解釋嗎？如果改以活躍的編輯者的數量來評估呢？
  ...
  - 其實人類的整體知識是增長的：「carrying capacity as a function of time」！但，如何評估上述的議題呢？目前這還沒有答案，値得去研究依然持續增長的 logistic growth model。
...
- 四個挑戰
  ...
  - 工程面
    ...
    - 重點：考慮到能够證明有顯著差異的樣本數（檢力），實際上需要蒐集到足够證明有差異的時間，改變色調的方案反而需要比較久！才知道到底有沒有差！但是這件事情很容易被業主忽略
  ...
  - 真實面
    - 實驗限制：隨機抽樣、隨機實驗、成本
    - 組織架構：其他組織的平行試驗、誰買單、能够造成 impact 嗎？、辦公室政治

感想：我覺得在很多層次上，公共領域的隨機對照實驗都有隨機是否足够隨機的問題！後面的四個挑戰收穫頗多！平常看 A/B test 的範例時，我都忘記要注意檢力的數量其實也是要考慮的重點。三個案例部分都是網路上大概比較有名的案例，覺得普普。
...
- 資金管理
- 博弈理論與交易
  - 實際上，因為交易次數遠小於收斂到平均値需要的次數，所以交易結果受隨機影響的機率影響極大。
...
- 銀行資料倉儲（供分析）的建設重點
  - 格式統一化
  - 資料正確性與 index 的唯一性（例如：身分證字號，够唯一嗎）
...
- Case: flight delayed
- 建議：盡早開始練習專案，並挑選一個生活相關的題目，例如以 opendata 形式開放的健康、交通、汙染資料等。
- Case: food
